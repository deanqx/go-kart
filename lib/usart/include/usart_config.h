...
// code #define USART_PUTHEX_IN_UPPERCASE // use uppercase letters in
// uart_puthex() function #define USART_EXTEND_RX_BUFFER // extend RX buffer by
// hardware 2/3 byte FIFO // required for hardware and software RTS #define
// USART_PUTC_FAST_INSERTIONS // skip FIFO procedure and write directly data to
// the UDR register when possible // probably required for full bus utilization
// at highest speed (f_cpu/8) #define USART_NO_LOCAL_BUFFERS // do not allocate
// temporary buffers on stack for integer/float <-> asci conversions and use
// globally visible u_tmp_buff[] instead // it have to be declared in
// application part and have to be at least of 6-17 bytes wide (depending on
// what is being converted) #define USART_UNSAFE_TX_INTERRUPT // max 19 cycles
// of interrupt latency // 3+PC bytes on stack // will not interrupt itself
// #define USART_UNSAFE_RX_INTERRUPT // max 23 cycles of interrupt latency //
// 4+PC bytes on stack // will not interrupt itself #define
// USART_REMAP_LAST_INTERFACE // remap hardware registers of USART1/2/3 to
// USART0 if only one interface is used #define USART_SKIP_UBRRH_IF_ZERO // do
// not generate code for writing to ubrrh if calculated value is zero //
...
// from every isr run // requires one globally reserved lower register #define
// USART_USE_GLOBALLY_RESERVED_ISR_Z_SAVE    // prematures out 6 cycles from
// every isr run // requires pair of globally reserved lower registers
